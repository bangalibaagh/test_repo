...
def test_create_device_type_returns_201_and_body(client):
    """Test that creating a device type returns 201 and the correct body.
...
def test_get_device_type_by_id_returns_200(client):
    """Test that getting a device type by ID returns 200 and the correct record.

    Args:
        client: The shared test client fixture.
    """
    create_resp = client.post("/device-types/", json={"name": "Switch"})
...
def test_create_duplicate_device_type_returns_409(client):
    ...
    Args:
        client: The shared test client fixture.
    """
    client.post("/device-types/", json={"name": "Hub"})
    response = client.post("/device-types/", json={"name": "Hub"})
    assert response.status_code == 409


def test_update_device_type_returns_200_with_changed_fields(client):
    """Test that updating a device type returns 200 and the updated fields.
    ...
    device_type_id = create_resp.json()["id"]
    response = client.put(
        f"/device-types/{device_type_id}", json={"name": "NewName", "description": "Updated"}
    )
    assert response.status_code == 200
    body = response.json()
...
def test_get_device_type_after_delete_returns_404(client):
    ...
    Args:
        client: The shared test client fixture.
    """
    create_resp = client.post("/device-types/", json={"name": "Ephemeral"})
    device_type_id = create_resp.json()["id"]
