...
class TestNowScript:
    ...
    def test_get_utc_timestamp_parseable(self):
        """Test that the returned timestamp can be parsed back to a datetime object."""
        timestamp = now.get_utc_timestamp()
        
        # Should be able to parse it back without errors
        parsed = datetime.datetime.fromisoformat(timestamp)
        assert parsed.tzinfo == datetime.timezone.utc

    def test_get_utc_timestamp_recent(self):
    ...
    def test_main_function(self, capsys):
        """Test that the main function prints the timestamp."""
        now.main()
